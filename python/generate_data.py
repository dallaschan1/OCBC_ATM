...
for user_id in range(1, 13):  # For all users
    if user_id not in suspicious_users:
        ...
        incoming = np.where(
            np.random.rand(len(user_indices)) < 0.5,  # 50% chance for each range
            np.random.uniform(1, 100, size=len(user_indices)),
            np.random.uniform(100, 500, size=len(user_indices))
        )
        outgoing = np.where(
            np.random.rand(len(user_indices)) < 0.5,
            np.random.uniform(1, 100, size=len(user_indices)),
            np.random.uniform(100, 500, size=len(user_indices))
        )

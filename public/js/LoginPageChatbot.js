...
async function processUserInput(input) {
    ...
    try {
        const response = await fetch('http://localhost:3001/loginPageChat', {
            method: 'POST',
        ...
        // Handle response
        if (aiResponse.includes("Fingerprint Login 3")) {
            window.location.href = "/fingerprint";
        } else if (aiResponse.includes("Face Recognition Login 2")) {
            window.location.href = "/faceLogin";
        } else if (aiResponse.includes("Display QR Code 4")) {
            window.location.href = "/display-qr";
        } else if (aiResponse.includes("Login 1")) {
            alert("Login using Card and PIN selected.");
        } else {
